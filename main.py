...
criterion1 = nn.CosineSimilarity(dim=1, eps=1e-7)
...
def train_gin(args, model, device, graphs, optimizer, epoch):
    ...
    for i in range(0, len(train_graphs), minibatch_size):
        ...
        loss = criterion(output_h, labels_h)
...
def train(args, model, patmem, device, graphs, samples, optimizer, optimizer_p, epoch):
    ...
    for i in range(0, len(train_graphs), minibatch_size):
        ...
        loss_d = (criterion1(graph_repre_tail, patterns_tail).sum() + criterion1(graph_repre_head, patterns_head).sum()) / n
...
def test(args, model, patmem, device, graphs, epoch):
    ...
    mask_t = mask_t.bool()
    loss =  criterion(output[mask_t], labels[mask_t])
